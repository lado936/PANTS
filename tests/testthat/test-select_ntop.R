...
test_that("ker", {
  #noker
  sn <- select_ntop(zscores=feature.stats[, "z", drop=FALSE], Gmat=G, pwy="pwy1", ker=noker, ntop = 3)
  expect_equal(nrow(sn), 3)
  expect_true(all(sn$in.pwy))

  #ker
  #a isn't in pwy2 but impacts it
  sn.z <- select_ntop(zscores=feature.stats[, "z", drop=FALSE], Gmat=G, pwy="pwy2", ker=kk, ntop = 3)
  ...
  sn.z <- select_ntop(zscores=feature.stats[, "z", drop=FALSE], Gmat=G, pwy="pwy1", ker=noker, ntop = 4)
...
test_that("impact = Ki*Gj*zi", {
  sn.sc <- select_ntop(zscores=feature.stats[, "score",drop=FALSE], Gmat=G, pwy="pwy1", ker=kk, ntop = 4)
  sn.z <- select_ntop(zscores=feature.stats[, "z", drop=FALSE], Gmat=G, pwy="pwy1", ker=kk, ntop = 4)
