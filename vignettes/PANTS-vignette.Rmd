...
res.lst <- pants(object=M, phenotypes.v=pheno.v, contrasts.v=contr, ker=ker, Gmat=smpdb_gmat, nperm=50)
pwy.stats <- res.lst$pwy.stats
```

## Plot
Draw network diagram of driving nodes for the pathway "Angiotensin Metabolism".
```{r plot}
score.v <- setNames(res.lst$feature.stats$score, nm=rownames(res.lst$feature.stats))
plot_pwy(gr=gr, ker=ker, Gmat=smpdb_gmat, pwy="Angiotensin Metabolism", score.v=score.v, annot=NA, ntop=3, name=NA)
```
