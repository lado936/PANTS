...
res.lst <- pants(object=M, phenotype.v=pheno.v, contrasts.v=contr, ker=ker, Gmat=smpdb_gmat, nperm=50)
pwy.stats <- res.lst$pwy.stats
```

## Plot
Draw network diagram of driving nodes for the pathway "Beta-Alanine Metabolism".
```{r plot}
score.v <- stats::setNames(res.lst$feature.stats$score, nm=rownames(res.lst$feature.stats))
plot_pwy(gr=gr, ker=ker, Gmat=smpdb_gmat, pwy="Beta-Alanine Metabolism", score.v=score.v, annot=NA, ntop=5, name=NA)
```
