...
## Construct graph
`PANTS` smooths statistics over the network using a network kernel derived from a graph. So we transform `pc9` into a graph:
```{r gr}
gr <- graph_from_edgelist(as.matrix(pc9[,c(1,3)]), directed = FALSE)
```
...
## Expand graph
We expand the graph to include nodes in `gmat`.
```{r eg}
gr <- expand_graph(gr, gmat)
```
...
```{r}
score.v <- setNames(res.lst$feature.stats$score, nm=rownames(res.lst$feature.stats))
plot_pwy(gr=gr, ker=ker, Gmat=gmat, pwy="Tryptophan Metabolism", score.v=score.v, annot=NA, ntop=3, name.pdf=NA)
```
