#' Plot colorbar
#' 
#' Plot colorbar. There are many functions for this in other pacakges, but they do not appear to handle \code{NA}s.
#' 
...
color_bar <- function(cols, lev, signif.dig=2, plot=TRUE){
  ...
  # e.g. usr = c(x1=-1, x2=1, y1=-5, y2=5)
  bx <- stats::setNames(graphics::par("usr"), nm=c("x1", "x2", "y1", "y2"))
  # box to right of plotting region
