...
match_mats <- function(score.mat, ker, Gmat, score.impute = 0) {
    stopifnot(length(intersect(rownames(score.mat), rownames(ker))) > 0, length(intersect(rownames(score.mat), 
        rownames(Gmat))) > 0)
    ...
    # add features to scores as zero matrix at bottom don't want to reorder this since it's the most dense
    ...
    if (new.ker.n > 0) {
        ker <- rbind(cbind(ker, Matrix::Matrix(data = 0, nrow = nrow(ker), ncol = new.ker.n)), cbind(Matrix::Matrix(data = 0, 
            nrow = new.ker.n, ncol = ncol(ker)), Matrix::Diagonal(n = new.ker.n)))
