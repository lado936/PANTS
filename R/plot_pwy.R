...
plot_pwy <- function(gr, ker, Gmat, pwy, score.v, annot = NA, ntop = 7, alternative = c("two.sided", "less", "greater"), 
    ...
    if (is.null(name)) 
        name <- paste0(ezlimma::clean_filenames(pwy), "_ntop", ntop)
    ...
    score.v <- mm$score.mat[, 1, drop = FALSE]
    ker <- mm$ker
    Gmat <- mm$Gmat
    ...
    # estimate impact of nodes on pwy score this includes estimation of impact.v to NA nodes, which are properly
    # handled by order() below
    impact.v <- stats::setNames((coeff.sc * score.v)[, 1], nm = rownames(score.v))
    
    # get top n nodes by abs(impact.v)
    top.nodes <- names(impact.v)[switch(alternative, greater = order(-impact.v), less = order(impact.v), two.sided = order(-abs(impact.v)))][1:ntop]
    ...
    pwy.nodes.ss <- union(intersect(pwy.nodes, top.nodes), intersect(pwy.nodes, neighbor_nms(gr, pwy.neighbors.ss)))
    ...
    if (plot) {
        ...
        if (any(shape.v == out.shape)) 
            graphics::legend(x = "topright", legend = c("Inside pwy", "Outside pwy"), pch = 1:0, bty = "n")
        if (!is.na(name)) 
            grDevices::dev.off()
    }
