...
pants <- function(object, phenotype.v, contrast.v, Gmat, ker = NULL, score_fcn = identity, nperm = 10^4, ret.null.mats = FALSE, 
    ...
    stopifnot(length(intersect(rownames(ker), rownames(object))) > 0, any(rownames(Gmat) %in% colnames(ker)), colnames(object) == 
        names(phenotype.v))
    ...
    for (perm in 1:nperm) {
        ...
        score.mat[, perm] <- score_features(object = object, phenotype.v = pheno.tmp, contrast.v = contrast.v, 
            score_fcn = score_fcn)
        if (verbose) {
            if (perm%%500 == 0) 
